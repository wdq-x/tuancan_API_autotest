...
工作流会在每次回归结束后，将 `allure-results` 原始结果发送到团餐管理平台。平台会解析用例状态、套件层级、耗时和失败信息，并在“经营管理 -> 测试管理 -> 自动化报告”中展示。该步骤不会上传账号密码或测试请求内容。

在 GitHub 仓库的 **Settings -> Secrets and variables -> Actions** 中继续配置：

- `TEST_REPORT_INGEST_URL`：平台接收接口完整地址，例如 `https://your-platform.example.com/v1/test-execution/ci-allure-report`。
- `TEST_REPORT_INGEST_TOKEN`：随机生成的共享令牌。
- `TEST_REPORT_ENV_CONFIG_ID`：可选，平台测试环境配置 ID；不配置时报告仍会入库。

平台服务的运行环境中必须配置同一个 `CI_REPORT_INGEST_TOKEN`。当上报地址或令牌缺失时，工作流会保留现有 Allure Artifact 和 GitHub Pages 发布流程，仅跳过平台上报步骤。
